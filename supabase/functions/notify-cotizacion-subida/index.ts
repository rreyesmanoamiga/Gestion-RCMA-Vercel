...
async function sendEmail(to: string, cc: string[], subject: string, html: string) {
  ...
  const msg = ['From: Sistema RCMA <' + smtpUser + '>', 'To: ' + to, ...(ccHeader ? ['Cc: ' + ccHeader] : []), 'Subject: ' + subject, 'MIME-Version: 1.0', 'Content-Type: text/html; charset=UTF-8', '', html, '.'].join('\r\n');
...
serve(async (req) => {
  ...
  try {
    ...
    await sendEmail(adminEmail, ccList, '📎 Cotizaciones subidas — ' + proyecto + ' | ' + centro, html);
