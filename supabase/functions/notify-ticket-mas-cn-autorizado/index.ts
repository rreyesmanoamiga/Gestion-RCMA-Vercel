...
serve(async (req) => {
  ...
  try {
    const { evento, folio, colegio, solicitante, correo_solicitante, correo_car, descripcion, concepto } = await req.json();
    const adminEmail = Deno.env.get('ADMIN_EMAIL') ?? 'rreyes@manoamiga.edu.mx';
    const smtpUser   = Deno.env.get('SMTP_USER')   ?? '';

    const esAutorizado = evento === 'autorizado';
    const to = esAutorizado ? (correo_solicitante || adminEmail) : adminEmail;
    const cc = esAutorizado ? [correo_car ?? ''] : [];
    const colorBanda = esAutorizado ? '#059669' : '#ED7102';
    const tituloBanda = esAutorizado ? `✅ Ticket MAS CN Autorizado — ${folio ?? ''}` : `📋 Nuevo Ticket MAS CN — ${folio ?? ''}`;
    const cuerpo = esAutorizado
      ? `Su Ticket MAS CN ha sido <strong style="color:${colorBanda};">revisado y autorizado</strong> por la Coordinación.`
      : `Se generó un nuevo Ticket MAS CN pendiente de revisión.`;
    ...
    <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
      <tr><td style="background:#00295A;padding:32px 40px;border-bottom:3px solid ${colorBanda};">
      ...
      <tr><td style="background:${colorBanda};padding:16px 40px;"><p style="margin:0;color:#fff;font-size:14px;font-weight:600;">${tituloBanda}</p></td></tr>
      <tr><td style="padding:40px;">
        <p style="color:#475569;font-size:15px;margin:0 0 8px;">Estimado(a) <strong>${solicitante ?? 'equipo'}</strong>,</p>
        <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 24px;">${cuerpo}</p>
        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;">
          ...
          <tr style="background:#f8fafc;"><td style="padding:10px 16px;font-size:12px;font-weight:700;color:#64748b;">Concepto</td><td style="padding:10px 16px;font-size:14px;">${concepto ?? '—'}</td></tr>
          <tr><td style="padding:10px 16px;font-size:12px;font-weight:700;color:#64748b;">Descripción</td><td style="padding:10px 16px;font-size:13px;color:#475569;">${descripcion ?? '—'}</td></tr>
        </table>
      </td></tr>
    ...
    await sendEmail(to, cc, tituloBanda, html);
