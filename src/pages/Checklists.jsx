...
// Eliminamos las importaciones de la carpeta UI que no existe
...
export default function Checklists() {
  ...
  // Clases de estilo para inputs y selects estándar
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(checklist => {
            ...
            return (
              <Link key={checklist.id} to={`/checklists/${checklist.id}`} className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-lg transition-all duration-300 group">
                <div className="flex items-start justify-between mb-3">
                  <StatusBadge status={checklist.overall_status} />
                  <div className="text-right shrink-0 ml-2">
                    {checklist.colegio && <p className="text-xs font-bold text-slate-800">{checklist.colegio}</p>}
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">
                      {checklist.infrastructure_type === 'concreto' ? 'Concreto' : 'Metálica'}
                    </span>
                  </div>
                </div>
                <h3 className="font-semibold text-slate-900 mb-1 group-hover:text-blue-600 transition-colors">{checklist.title}</h3>
                {checklist.project_id && (
                  <p className="text-xs text-slate-500 mb-2 truncate">Proyecto: {getProjectName(checklist.project_id)}</p>
                )}
