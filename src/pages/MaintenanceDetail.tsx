...
import { ArrowLeft, Pencil, Trash2, MapPin, Calendar, User, DollarSign, AlertTriangle } from 'lucide-react';
  ...
  if (isError || !record) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-500">
        ...
        <p className="text-sm">No se encontró el registro de mantenimiento.</p>
        <Link to="/mantenimiento" className="text-sm text-blue-600 hover:underline">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            ...
            {projectName && (
              <p className="text-sm text-slate-500 mt-1 font-medium">Proyecto: {projectName}</p>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm bg-slate-50 p-4 rounded-lg">
          ...
          {record.responsible && (
            <div className="flex items-center gap-2 text-slate-600">
              <User className="w-4 h-4 text-slate-400" />{record.responsible}
            </div>
          )}
          {record.scheduled_date && (
            <div className="flex items-center gap-2 text-slate-600">
              <Calendar className="w-4 h-4 text-slate-400" />
              Prog: {format(new Date(record.scheduled_date), 'dd MMM yyyy', { locale: es })}
            </div>
          )}
          {record.completed_date && (
            <div className="flex items-center gap-2 text-slate-600">
              <Calendar className="w-4 h-4 text-slate-400" />
              Comp: {format(new Date(record.completed_date), 'dd MMM yyyy', { locale: es })}
            </div>
          )}
          {record.cost && (
            <div className="flex items-center gap-2 text-slate-600 font-semibold">
              <DollarSign className="w-4 h-4 text-slate-400" />
              {Number(record.cost).toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}
            </div>
          )}
          {record.next_maintenance_date && (
            <div className="flex items-center gap-2 text-blue-600 font-medium">
              <Calendar className="w-4 h-4" />
              Próx: {format(new Date(record.next_maintenance_date), 'dd MMM yyyy', { locale: es })}
            </div>
          )}
        </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
              {record.photos_before.map((url, i) => (
                <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                  className="aspect-square rounded-lg overflow-hidden border border-slate-200 hover:ring-2 hover:ring-slate-300 transition-all">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-sm w-full p-6">
            ...
            <p className="text-sm text-slate-500 mt-2">
              Esta acción no se puede deshacer y el registro de mantenimiento desaparecerá del sistema.
            </p>
            <div className="flex justify-end gap-3 mt-6">
              ...
              >
                {deleteMutation.isPending ? 'Eliminando...' : 'Eliminar Registro'}
              </button>
            </div>
          </div>
        </div>
