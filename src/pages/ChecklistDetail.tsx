...
async function loadJsPDF() {
  ...
  if (w.jspdf?.jsPDF) return w.jspdf.jsPDF as typeof import('jspdf').jsPDF;
  ...
  return (w.jspdf as { jsPDF: typeof import('jspdf').jsPDF }).jsPDF;
}
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {checklist.infrastructure_type && (
              <span className="px-2 py-0.5 rounded border text-[10px] font-black uppercase tracking-wider bg-slate-100 text-slate-600 border-slate-200">
                {TIPOS_MATERIAL[checklist.infrastructure_type] || checklist.infrastructure_type}
              </span>
            )}
          </div>
