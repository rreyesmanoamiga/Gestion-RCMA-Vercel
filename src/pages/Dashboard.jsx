...
export default function Dashboard() {
  const { data: projects = [] } = useQuery({
    queryKey: ['projects'],
    queryFn: () => db.Project.list('-created_at', 50),
  });

  const { data: checklists = [] } = useQuery({
    queryKey: ['checklists'],
    queryFn: () => db.Checklist.list('-created_at', 50),
  });

  const { data: maintenance = [] } = useQuery({
    queryKey: ['maintenance'],
    queryFn: () => db.MaintenanceRecord.list('-created_at', 50),
  });

  const activeProjects = projects.filter(p => p.status === 'en_progreso').length;
  const criticalChecklists = checklists.filter(c => c.overall_status === 'critico' || c.overall_status === 'malo').length;
  const pendingMaintenance = maintenance.filter(m => m.status === 'pendiente').length;
  const urgentItems = maintenance.filter(m => m.priority === 'urgente' && m.status !== 'completado').length;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Proyectos Activos" value={activeProjects} subtitle={`${projects.length} total`} icon={FolderKanban} color="blue" />
        <StatCard title="Inspecciones" value={checklists.length} subtitle={criticalChecklists > 0 ? `${criticalChecklists} requieren atención` : 'Todo en orden'} icon={ClipboardCheck} color="green" />
        <StatCard title="Mtto. Pendiente" value={pendingMaintenance} subtitle={`${maintenance.length} registros totales`} icon={Wrench} color="orange" />
        <StatCard title="Urgentes" value={urgentItems} subtitle="Requieren acción inmediata" icon={AlertTriangle} color="red" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {['NORTE', 'MEXICO'].map(territorio => {
          ...
          const colegios = COLEGIOS.filter(c => c.territorio === territorio);
          return (
            <div key={territorio} className="bg-card rounded-xl border border-border p-5">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                ...
                <div className="text-center p-2 bg-muted/50 rounded-lg">
                  <p className="text-xl font-bold text-foreground">{tMaint.filter(m => m.status !== 'completado').length}</p>
                  <p className="text-[10px] text-muted-foreground">Mtto. Pend.</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-1">
                {colegios.map(c => {
                  const hasUrgent = projects.some(p => p.colegio === c.colegio && p.priority === 'urgente' && p.status !== 'completado')
                    || maintenance.some(m => m.colegio === c.colegio && m.priority === 'urgente' && m.status !== 'completado')
                    || checklists.some(cl => cl.colegio === c.colegio && (cl.overall_status === 'critico' || cl.overall_status === 'malo'));
                  return (
                    <span key={c.colegio} className={`text-xs px-2 py-0.5 rounded-md font-medium border ${hasUrgent ? 'bg-red-100 text-red-700 border-red-200' : 'bg-muted text-muted-foreground border-border'}`}>
                      {c.colegio}
                    </span>
                  );
                })}
        ...
        <div className="bg-card rounded-xl border border-border">
          ...
          <div className="divide-y divide-border">
            ...
            {recentProjects.map(project => (
              <Link key={project.id} to={`/proyectos/${project.id}`} className="flex items-center justify-between p-4 hover:bg-muted/50 transition-colors">
        ...
        <div className="bg-card rounded-xl border border-border">
          ...
          <div className="divide-y divide-border">
            ...
            {recentMaintenance.map(record => (
              <Link key={record.id} to={`/mantenimiento/${record.id}`} className="flex items-center justify-between p-4 hover:bg-muted/50 transition-colors">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground truncate">{record.title}</p>
                  <p className="text-xs text-muted-foreground">
                    {record.scheduled_date ? format(new Date(record.scheduled_date), 'dd MMM yyyy', { locale: es }) : 'Sin fecha'}
                  </p>
                </div>
                <div className="flex items-center gap-2 ml-3">
